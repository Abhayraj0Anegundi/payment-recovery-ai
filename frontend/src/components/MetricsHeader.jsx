...
function StatCard({ label, value, decimals = 0, suffix = '', sublabel, tone = 'default', featured = false }) {
  ...
  return (
    <div
      className={`rounded-2xl px-5 py-4 flex-1 min-w-[160px] transition-all hover:shadow-md ${
        featured
          ? 'bg-gradient-to-br from-brand-600 to-brand-700 shadow-sm shadow-brand-600/30'
    ...
    >
      ...
      >
        <AnimatedNumber
          ...
          decimals={decimals}
          suffix={suffix}
          flashClassName={featured ? 'text-teal-200 scale-110' : 'text-brand-600 scale-110'}
        />
      </div>
      {sublabel && (
        <div className={`text-xs mt-1 ${featured ? 'text-brand-100/90' : 'text-slate-400'}`}>{sublabel}</div>
      )}
    </div>
  )
}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl px-5 py-4">
        ...
        <div className="flex items-center gap-2">
          {[
            ...
            { label: 'Contacted', value: summary.funnel.contacted, color: 'bg-brand-500' },
            { label: 'Recovered', value: summary.funnel.recovered, color: 'bg-teal-500' },
          ].map((stage, i, arr) => {
