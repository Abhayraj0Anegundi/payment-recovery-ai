      ...
      <div className="divide-y divide-white/10">
        {messages.map((m) => (
          <div key={m.transaction_id} className="grid grid-cols-1 md:grid-cols-2 gap-0">
            <div className="px-5 py-4 bg-white/[0.02] border-r border-white/10">
            ...
            <div className="px-5 py-4 bg-teal-400/[0.06]">
              <div className="flex items-center gap-2 mb-1.5">
                <span className="text-[10px] font-semibold text-teal-300 uppercase tracking-wide">
                  Actual output — {CAUSE_LABELS[m.root_cause] || m.root_cause}
                </span>
                <span className="text-[10px] text-slate-500">#{m.transaction_id} · {m.customer_name}</span>
              </div>
              <p className="text-sm text-slate-200">{m.message_text}</p>
            </div>
          </div>
        ))}
      </div>
